...
window.Tether = __webpack_require__(5);
  ...
  */
	var $addRows = function () {
		...
		var $trigger = $('[data-add-row]'),
		    setRowsInput = function setRowsInput() {
  ...
  */
	$('[data-trumbo]').each(function () {
		...
		if (typeof $this.data('trumbo-config') === 'undefined') {
			var $buttons = [['undo', 'redo'], ['formatting'], ['strong', 'em', 'del'], ['superscript', 'subscript'], ['link'], ['justifyLeft', 'justifyCenter', 'justifyRight', 'justifyFull'], ['unorderedList', 'orderedList'], ['horizontalRule'], ['removeformat'], ['fullscreen']];
		} else {
		...
		if ($imageTarget.length > 0) {
			...
			if ($form.length > 0) {
				...
				if ($input.length > 0) {
					...
					$form.prepend($('<input>').prop('type', 'hidden').prop('name', 'media_id').val($this.data('media-id')));
				}
			}
		}
		...
		$.ajax({
			...
			success: function success(response) {
				if (response.hasOwnProperty('taxonomy')) {
					var $li = $('<li>');
					var $input = $('<input>').prop('type', 'checkbox').prop('id', 'tax-' + response.taxonomy.id).prop('name', 'taxonomies[]').prop('checked', true).val(response.taxonomy.id);
					var $label = $('<label>').prop('for', 'tax-' + response.taxonomy.id).text(response.taxonomy.name);
...
var __WEBPACK_AMD_DEFINE_FACTORY__, __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;/*! jQuery UI - v1.12.1 - 2018-05-10
* http://jqueryui.com
